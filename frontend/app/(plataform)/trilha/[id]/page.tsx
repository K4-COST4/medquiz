...
export default function TrilhaMapPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ---------------------------------------------------------
  // 1. CARREGAR OS TÓPICOS (Para a Roleta Inferior)
  // ---------------------------------------------------------
  useEffect(() => {
  ...
  useEffect(() => {
    async function fetchContent() {
      ...
      try {
        ...
        // A. Buscar OBJETIVOS (As seções dentro da unidade)
        const { data: objectivesData } = await supabase
          .from('study_nodes')
          .select('*')
          .eq('parent_id', activeTopic.id)
        ...
        // D. LÓGICA DE BLOQUEIO CORRIGIDA
        // Regra: Precisa ter completado o anterior pelo menos 1 vez (nível >= 1) para liberar o próximo
        // EXCETO: Bosses são SEMPRE clicáveis (para o Jump Ahead)
  ...
  const handleTopicChange = (direction: 'next' | 'prev') => {
  ...
  // Calcula o strokeDashoffset para a barra circular
  ...
  if (topics.length === 0) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-slate-400 mt-2">Nenhum tópico encontrado para esta área.</p>
                            ...
                            section.nodes.map((node) => {
                                ...
                                <div key={node.id} className={`relative z-10 ${offsetClass}`}>
                                    ...
                                    >
                                        ...
                                        {/* Anel de Progresso (Nível 1 = 33%, Nível 2 = 66%) */}
         ...
         <div className="max-w-[1050px] mx-auto lg:grid lg:grid-cols-[1fr_360px] gap-8">
            <div className="lg:col-span-1 pointer-events-auto">
                <div className="bg-white/90 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-2xl p-3 flex items-center justify-between max-w-md mx-auto">
                    <button 
                      onClick={() => handleTopicChange('prev')}
                      disabled={currentTopicIndex === 0}
                      className="p-3 hover:bg-slate-100 rounded-xl text-slate-400 disabled:opacity-30 transition-colors"
                    >
                    ...
                    <button 
                      onClick={() => handleTopicChange('next')}
